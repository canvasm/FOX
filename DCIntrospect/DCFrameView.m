...
@synthesize outlineColors;

- (void)dealloc
{
	self.delegate = nil;
    [selectedColor release];
    [outlineColors release];
...
- (id)initWithFrame:(CGRect)frame delegate:(id)aDelegate
{
	...
	if (self)
	{
        ...
        self.outlineColors = [NSArray arrayWithObjects:[UIColor blueColor], [UIColor redColor], [UIColor whiteColor], nil];
...
- (void)cycleOutlineColors;
{
    int selectedIndex = (([self.outlineColors indexOfObject:self.selectedColor] + 1) % self.outlineColors.count);
    self.selectedColor = [self.outlineColors objectAtIndex:selectedIndex];
}
