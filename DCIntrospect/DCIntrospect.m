...
#define RENAMED_VALUE_FOR_KEY_SELECTOR NSSelectorFromString(@"swizzledTextInputTraitsValueForKey:")
...
id UITextInputTraits_valueForKey(id self, SEL _cmd, NSString *key)
{
    ...
    id (*originalValueForKey)(id, SEL, NSString *) = (id (*)(id, SEL, NSString *))class_getMethodImplementation([self class], RENAMED_VALUE_FOR_KEY_SELECTOR);
	
	if ([textInputTraitsProperties containsObject:key])
	{
		id textInputTraits = originalValueForKey(self, _cmd, @"textInputTraits");
		return [textInputTraits valueForKey:key];
	}
	else
	{
		return originalValueForKey(self, _cmd, key);
	}
}
...
+ (void)workaroundUITextInputTraitsPropertiesBug
{
	...
	for (unsigned int i = 0; i < count; i++)
	{
		...
		{
            Method valueForKeyMethod = class_getInstanceMethod(class, @selector(valueForKey:));
            IMP valueForKeyIMP = method_getImplementation(valueForKeyMethod);
            
            if (valueForKeyIMP == (IMP)&UITextInputTraits_valueForKey)
                continue;
            
            if (!class_addMethod(class, RENAMED_VALUE_FOR_KEY_SELECTOR, valueForKeyIMP, method_getTypeEncoding(valueForKeyMethod))) {
                NSLog(@"Could not rename valueForKey: to %@ for class %@", NSStringFromSelector(RENAMED_VALUE_FOR_KEY_SELECTOR), class);
                continue;
            }
            
            class_replaceMethod(class, method_getName(valueForKeyMethod), (IMP)&UITextInputTraits_valueForKey, method_getTypeEncoding(valueForKeyMethod));
		}
	}
	free(classes);
}
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)string
{
  ...
  if ([string isEqualToString:kDCIntrospectKeysToggleViewOutlines])
    ...
    else if ([string isEqualToString:kDCIntrospectKeysToggleSubviewOutlines])
    {
        [self toggleOutlinesForSelectedView];
        return NO;
    }
    ...
    else if ([string isEqualToString:kDCIntrospectKeysCycleSelectedColors])
    {
        [self.frameView cycleOutlineColors];
        [self updateFrameView];
	...
	if (self.on && self.currentView)
	{
        ...
        else if ([string isEqualToString:kDCIntrospectKeysLogFont])
		{
			[self showTemporaryStringInStatusBar:[self fontForCurrentView:self.currentView]];
			return NO;
		}
...
- (void)toggleOutlinesForSelectedView
{
    self.viewOutlines = !self.viewOutlines;
	
	if (self.viewOutlines)
		[self addOutlinesToFrameViewFromSubview:self.currentView];
	else
		[self.frameView.rectsToOutline removeAllObjects];
...
- (void)logAccessabilityPropertiesForObject:(id)object
{
    ...
    [outputString appendFormat:@"%@", [self accessibilityPath:object]];
...
- (NSString *)accessibilityPath:(id)object
{
    NSMutableArray *stringArray = [NSMutableArray array];
    NSMutableString *accessibilityPath = [NSMutableString string];
    [accessibilityPath appendFormat:@"	Accesibility path:"];
    
    void (^addObjectAccessibilityString)(id) = ^(id obj){
        if ([obj accessibilityIdentifier]) {
            [stringArray addObject:[NSString stringWithFormat:@"(%@):%@", [obj class], [obj accessibilityIdentifier]]];
        } else if ([obj accessibilityLabel]) {
            [stringArray addObject:[NSString stringWithFormat:@"(%@):%@", [obj class], [obj accessibilityLabel]]];
    ...
    BOOL stop = NO;
    do {
        addObjectAccessibilityString(tempObj);
    
        
        if (![tempObj superview]) {
            [stringArray addObject:[NSString stringWithFormat:@"(%@):%@", [tempObj class], [tempObj accessibilityLabel]]];
            stop = YES;
        } else {
            tempObj = [tempObj superview];
        }
        
    } while (stop == NO);
    
    NSMutableArray *array = [NSMutableArray arrayWithCapacity:[stringArray count]];
    NSEnumerator *enumerator = [stringArray reverseObjectEnumerator];
    for (id element in enumerator) {
        [array addObject:element];
    }
    
    for (NSString *str in array) {
        if ([array lastObject] == str) {
            [accessibilityPath appendFormat:@" %@", str];
        } else {
            [accessibilityPath appendFormat:@" %@ |", str];
        }
    ...
    [accessibilityPath appendFormat:@"\n"];
...
- (NSString *)fontForCurrentView:(UIView *)view
{
    if ([view respondsToSelector:@selector(font)]) {
        return [[(UILabel *)view font] fontName];
    } else {
        return [NSString stringWithFormat:@"%@ does not have font property", view.class];
    }
}
@end
